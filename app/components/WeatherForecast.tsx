...
export default function WeatherForecast({ city, location }: WeatherForecastProps) {
  ...
  return (
    <div className="h-full rounded-2xl sm:rounded-3xl p-4 sm:p-4 md:p-5 relative overflow-hidden" style={{ background: 'linear-gradient(to bottom right, #CC9C75, #D5D8B5)' }}>
      ...
      <div className="relative z-10 h-full flex flex-col min-h-0 overflow-y-auto">
        <h2 className="text-xl sm:text-xl md:text-2xl font-black mb-3 sm:mb-4 drop-shadow-lg flex-shrink-0 text-center" style={{ color: '#2C2C2C' }}>
          5 Günlük Tahmin
        </h2>
        
        <div className="flex-1 flex flex-col gap-3 sm:gap-2.5 md:gap-3 min-h-0">
          {forecast.map((day, index) => {
            ...
            return (
              <div
                key={day.date}
                className="rounded-xl sm:rounded-2xl p-3 sm:p-3 border backdrop-blur-md transition-all duration-300 flex-shrink-0"
                style={{ 
